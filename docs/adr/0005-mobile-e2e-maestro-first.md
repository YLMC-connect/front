...
YLMC Connect v1은 Expo Dev Client 기반 모바일 앱이다. 수동 실행만으로는 Downloads preview 기준 홈/나눔/동행/기도/삶공부 5탭과 동행 내부 소모임/봉사 segment의 회귀를 안정적으로 잡기 어렵다.

## 결정

- 모바일 E2E smoke는 Maestro를 우선 사용한다.
- 테스트 대상은 Expo Go가 아니라 `com.ylmc.connect.dev` development build다.
- 최소 E2E smoke는 `.maestro/smoke.yml`로 홈, 나눔, 동행, 기도, 삶공부 5탭 진입을 확인한다.
