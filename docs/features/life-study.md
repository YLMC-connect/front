...
- 동행 삶공부 segment 실제 화면 복구 — `DesignSourceScreens` placeholder 대신 Downloads `ScreenStudyList`의 내 학습경로, 신청 가능한 과정, 전체 과정 구조를 `/faith?section=study` route에 RN으로 직접 반영
- 삶공부 direct route 실제 화면 복구 — `/life-study` 숨김 direct route도 placeholder 대신 `/faith` 삶공부 segment의 실제 RN 화면을 재사용하도록 반영
...
| `app/(tabs)/life-study/index.tsx`   | 숨김 direct route. `/faith` 삶공부 화면 재사용  |
...
- (2026-06-27) **삶공부 direct route는 실제 RN 화면을 재사용한다** — `/life-study`는 별도 복제 구현 없이 `/faith` 삶공부 segment 화면을 재사용합니다. 하단 탭 IA는 계속 `동행` 중심으로 유지합니다.
...
- (2026-06-27) **동행 삶공부 segment는 실제 RN 화면으로 렌더링한다** — Downloads `ScreenStudyList` 구조를 `/faith?section=study`에 직접 반영합니다. 기존 `/life-study` route는 숨김 direct route로 유지하며, 실제 API 스키마 확정 전까지 추가 mutation/helper는 만들지 않습니다.
