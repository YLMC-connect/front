...
- 2026-05-27 MY 차단/프로필 정렬 후 비교 리포트의 상위 residual은 signup variant, study-detail, market-list, 소모임 상세 일부입니다. `user-ts`는 `29.61→4.97`, `me-blocked-ts`는 `26.57→6.13`까지 낮췄으며, 다음 우선순위는 ZIP `ScreenSignup` variant와 study-detail 정렬입니다.
