...
- 나눔 플로팅 버튼 행동 문구 확인 — 공통 FAB의 명시적 action 규칙에 따라 기존 `글쓰기` 문구와 `/modal/market-new` 이동을 유지하고 접근성 이름을 같은 문구로 고정
...
- (2026-07-15) **나눔 FAB는 `글쓰기`를 행동 label로 유지한다** — 이미 작성 행동이 명확하므로 문구와 `+` 아이콘, `/modal/market-new` 이동, 고정 위치를 변경하지 않고 공통 FAB의 필수 label·접근성 규칙만 적용합니다.
