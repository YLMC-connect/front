...
- 동행 기본 segment 실제 화면 복구 — `DesignSourceScreens` placeholder 대신 Downloads `ScreenPrayerList`의 내 기도방, 내 기도제목, 중보기도 신청 구조를 `/faith` route에 RN으로 직접 반영
- 중보기도 direct route 실제 화면 복구 — `/prayer` 숨김 direct route도 placeholder 대신 `/faith` 기도 segment의 실제 RN 화면을 재사용하도록 반영
...
| `app/(tabs)/prayer/index.tsx`    | 숨김 direct route. `/faith` 기도 화면 재사용      |
...
- (2026-06-27) **중보기도 direct route는 실제 RN 화면을 재사용한다** — `/prayer`는 별도 복제 구현 없이 `/faith` 기도 segment 화면을 재사용합니다. 하단 탭 IA는 계속 `동행` 중심으로 유지합니다.
...
- (2026-06-27) **동행 기본 segment는 실제 RN 화면으로 렌더링한다** — Downloads `ScreenPrayerList` 구조를 `/faith` 기본 화면에 직접 반영합니다. 기존 `/prayer` route는 숨김 direct route로 유지하며, 실제 API 스키마 확정 전까지 추가 query helper는 만들지 않습니다.
