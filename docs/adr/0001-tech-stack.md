...
- **react-hook-form + zod + @hookform/resolvers**
  - 비제어 컴포넌트 기반 RHF 가 RN 에서 리렌더 비용이 가장 낮음.
  - **함정**: zod v4 + 기본 resolvers 조합은 TS 타입 추론이 깨진다 → `@hookform/resolvers ^5.2.2` 필수. PLAN.md 호환성 표에 명시됨.
...
- **date-fns 4** — 트리셰이킹 가능한 경량 날짜 유틸. moment / dayjs 대체.
...
| **Yup** (zod 대체) | TypeScript 타입 추론 품질에서 zod 우위. 스키마 = 타입 일원화 가치가 큼. |
