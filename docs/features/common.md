...
- 2026-05-27 회원가입 정렬 후 비교 리포트의 상위 residual은 study-detail, 소모임 상세 일부, market-list, notifications입니다. signup variant는 ZIP `ScreenSignup` 구조 반영으로 `signup-pw-error 21.74→10.14`, `signup-id-dup 20.65→9.18`, `signup default 12.50→7.76`까지 낮췄으며, 다음 우선순위는 ZIP `ScreenStudyDetail` 또는 market-list 정렬입니다.
