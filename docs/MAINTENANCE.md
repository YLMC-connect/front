...
> 마지막 갱신: 2026-05-22 (AI 작업 규칙 SSOT를 Codex `AGENTS.md` 로 전환) | 대상: 사람 신규 기여자 + AI 차기 세션 둘 다
...
> **트리거**: 사용자가 `유지보수 해줘`, `drift check`, `docs 점검` 등으로 호출하면 본 런북을 1 → 2 → 3 → 4 단계 순서로 수행합니다. 트리거 없이는 평소 작업의 [../AGENTS.md](../AGENTS.md) 의무 4종 갱신만 하면 됩니다.
